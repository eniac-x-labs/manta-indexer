...
func (h Routes) GetOperatorSharesDecreased(w http.ResponseWriter, r *http.Request) {
	guid := r.URL.Query().Get("guid")
	if guid == "" {
		http.Error(w, "invalid query params", http.StatusBadRequest)
		log.Error("error GetOperatorSharesDecreased reading request params")
		return
	}
	temp, err := h.svc.GetOperatorSharesDecreased(guid)
	if err != nil {
		http.Error(w, "Internal server error reading GetOperatorSharesDecreased", http.StatusInternalServerError)
		log.Error("Unable to read GetOperatorSharesDecreased from DB", "err", err.Error())
		return
	}
